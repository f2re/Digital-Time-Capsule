...
async def process_recipient(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # In v21.0+, forward_from_chat was removed. Instead, we check for:
    # 1. message.is_from_offline_bot - indicates forwarded from bot
    # 2. message.forward_origin - new way to detect forwards
    # 3. message.chat.type != 'private' and message.reply_to_message - reply in group
    
    chat_to_send = None
    is_forwarded = False
    
    # Method 1: Check if message has forward_origin (new API)
    ...
    # Method 2: Check if user replied to a message in current chat
    ...
    # Method 3: Check if message is from a group/channel (not private)
    ...
    if is_forwarded and chat_to_send:
        logger.info(f"User {user.id} selected chat {chat_to_send.id} ({chat_to_send.title})")
        ...
        try:
            ...
            if chat_to_send.type == 'channel':
                ...
                if not (hasattr(bot_member, 'can_post_messages') and bot_member.can_post_messages):
                    ...
                    await message.reply_text(t(lang, 'no_post_rights', chat_title=chat_to_send.title))
                    return PROCESSING_RECIPIENT
            ...
            await message.reply_text(t(lang, 'bot_not_in_chat', chat_title=chat_to_send.title or "Unknown"))
            return PROCESSING_RECIPIENT
        except Exception as e:
        ...
        context.user_data['capsule']['recipient_name'] = chat_to_send.title or f"Chat {chat_to_send.id}"
    ...
    # Case 3: User typed chat ID directly (for advanced users)
        ...
        try:
            ...
            context.user_data['capsule']['recipient_name'] = chat_info.title or f"Chat {chat_id}"
            return await show_confirmation(update, context)
