...
async def show_ideas_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not user_data:
        ...
        if not user_data:
            ...
            # Pass basic user data to avoid None issues
    ...
    try:
        if data == 'ideas_menu':
            ...
            try:
                ...
                # Message might be too old, send a new one
                await query.message.reply_text(t(lang, 'ideas_menu_title'), reply_markup=_category_keyboard(lang))
            return SELECTING_IDEAS_CATEGORY
        ...
        # Handle fallback cases for 'main_menu' and 'cancel'
        if data in ('main_menu', 'cancel'):
            from .start import show_main_menu_with_image
            user_data = get_user_data(user.id)
            return await show_main_menu_with_image(update, context, user_data)
        
        # If no pattern matched, return to main menu
